...
static struct 	expty transExp(Tr_level, S_table venv,S_table tenv,A_exp a);
//static void 	transDec(S_table venv,S_table tenv,A_dec d);
static void 	transDec(Tr_level, S_table v, S_table t, A_dec d);

static Ty_ty	transTy(S_table tenv,A_ty a);

static bool 	args_match(Tr_level, S_table v, S_table tt, A_expList ell, Ty_tyList fll, A_exp fun);
static bool 	ty_match(Ty_ty tt, Ty_ty ee);
static Ty_ty    actual_ty(Ty_ty ty);
static bool 	efields_match(Tr_level,S_table v, S_table t, Ty_ty tyy, A_exp e);
...
static struct expty transExp(Tr_level level,S_table v, S_table t, A_exp e){
	...
	switch(e->kind){
		...
		case A_arrayExp:
			...
			final2 = transExp(level,v, t, e->u.array.size);//数组大小 表达式
			final3 = transExp(level,v, t, e->u.array.init);//数组初始化 表达式
